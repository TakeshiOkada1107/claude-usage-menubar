...
3 分以上更新が無いと全体が淡色になり、数字の右に `(12分前)` のように経過が付く。
色の差だけでは見落とすため、いつの値かを数字で示している。メニューを開くと
「Claude Code が動いていないため古い値の可能性」と併せて出る。

**メニューバーの値が使用量ページと食い違うときは、まずここを見る。** レート制限は
Claude Code のセッションが動いたときにしか更新されないため、放置していた間に
claude.ai の Web や別のセッションで消費した分は、次にセッションが動くまで反映されない。
